...
function voiceMemoDurationWarning(status, limits = {}) {
  ...
  const maxSeconds = limits.maxSeconds ?? VOICE_MEMO_MAX_DURATION_SEC;
  if (status === "too_short") {
    return `Your voice memo is shorter than ${minSeconds} seconds. Please record again and resubmit a memo between ${minSeconds} seconds and ${Math.floor(maxSeconds / 60)} minutes.`;
  }
  if (status === "too_long") {
